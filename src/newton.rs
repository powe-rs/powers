...
use crate::rpower::{d_imis_d_v, d_sbus_d_v, SBus};
use crate::traits::LinearSolver;
use densetools::arr::{conj, Arr, CArr};
use densetools::slice::{norm_inf, select, set_slice};
...
pub(crate) fn newtonpf(
    ...
    while !converged && i < max_it {
        ...
        let (_, neg_d_sd_d_vm) = s_bus.s_bus2(&vm);
        d_sbus_d_vm = d_sbus_d_vm - neg_d_sd_d_vm.to_csr();
...
pub(crate) fn newtonpf_i_polar(
    ...
    // evaluate F(x0)
    let mut s_b = s_bus.s_bus(&vm);
    let mis: Arr<Complex64> = {
        let v_pv = v.select(&pv);
        let ybus_pv = y_bus.select(Some(&pv), None)?;
        let sb_pv = Arr::from_parts(
            &s_b.select(&pv).real(),
            &(v_pv * conj(&(ybus_pv * &v))).imag(),
        );
        s_b.set(pv, &sb_pv);
        (y_bus * &v) - conj(&(&s_b / &v))
    };
    ...
    let mut f = {
        ...
        Arr::concat(&[&mis_pvpq.real(), &mis_pvpq.imag()])
    };
    ...
    while !converged && i < max_it {
        ...
        */
        ...
        let dImis_dQ = Coo::new(
            ...
            pv.to_vec(),
            pv.to_vec(),
            (Complex64::i() / conj(&v_pv)).to_vec(),
        )?
        .to_csr();
        let (dImis_dVa, dImis_dVm) = d_imis_d_v(&s_b, &y_bus, &v, false)?;
        ...
        */
        ...
        if npv != 0 {
            ...
            */
            ...
            for (i, &j) in pv.iter().enumerate() {
                va[j] += dx_va[i];
                s_b[j] += Complex64::new(0.0, dx_sb[i]);
            }
        }
        if npq != 0 {
            /*
            ...
            */
            let dx_va = dx.select(&(j3..j4).collect::<Vec<usize>>());
            let dx_vm = dx.select(&(j7..j8).collect::<Vec<usize>>());
        ...
        let mis: Arr<Complex64> = (y_bus * &v) - conj(&(&s_b / &v));
...
pub(crate) fn newtonpf_i_cart(
    ...
    // evaluate F(x0)
    let mut sb = s_bus.s_bus(&vm);
    let sb_pv = Arr::from_parts(
        &sb.select(&pv).real(),
        &(v.select(&pv) * conj(&(y_bus * &v))).imag(),
    );
    sb.set(pv, &sb_pv);
    let mis = (y_bus * &v) - conj(&(&sb / &v));
    ...
    while !converged && i < max_it {
        ...
        let dV2_dVi = Coo::new(
            ...
            (0..npv).collect(),
            (npq..npq + npv).collect(),
            (2.0 * v_pv.imag()).to_vec(),
        )?
        .to_csr();
        let (dImis_dVr, dImis_dVi) = d_imis_d_v(&sb, &y_bus, &v, true)?;
        ...
        */
        ...
        // compute update step
        let dx: Arr<f64> = lin_solver.solve(jac.to_csc(), &(-&f))?;
        ...
        if npv != 0 {
            ...
            */
            let dx_v: Arr<Complex64> = Arr::from_parts(
            ...
            for (i, &j) in pv.iter().enumerate() {
                v[j] += dx_v[i];
                sb[j] += Complex64::new(0.0, dx_sb[i]);
            }
        }
        if npq != 0 {
            // V(pq) = V(pq) + dx(j3:j4) + 1j * dx(j7:j8);
            let dx_v = Arr::<Complex64>::from_parts(
...
pub(crate) fn newtonpf_I_hybrid(
    ...
    */
    ...
    let sb_pv = Arr::from_parts(
        &sb.select(&pv).real(),
        &(v.select(&pv) * conj(&(ybus_pv * &v))).imag(),
    );
    sb.set(pv, &sb_pv);
    let mis = (y_bus * &v) - conj(&(sb / &v));
